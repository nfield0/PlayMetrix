...
class GettingStartedPlayer extends ConsumerWidget {
  final introKey = GlobalKey<IntroductionScreenState>();
  final int playerId;
  final bool isOnSignUp;

  GettingStartedPlayer(
      {super.key, required this.playerId, this.isOnSignUp = false});

  void _onIntroEnd(context) {
    if (isOnSignUp) {
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(
            builder: (_) => PlayerProfileSetUpScreen(
                  playerId: playerId,
                )),
      );
    } else {
      ...
      IntroductionScreen(
        ...
        pages: [
          ...
          PageViewModel(
            ...
            decoration: pageDecoration,
            bodyWidget: showcaseBodyWidget(
              "player_showcase/6.png",
              context,
            ),
          ),
          PageViewModel(
            title: "Confirm your attendance for schedules",
            decoration: pageDecoration,
            bodyWidget: showcaseBodyWidget(
              "player_showcase/4.png",
              context,
            ),
          ),
          PageViewModel(
